...
Figma에서 추출할 프레임이나 레이어를 선택하고 macOS는 `Command L`, Windows는 `Ctrl L`을 눌러 selection 링크를 복사합니다. 우클릭 메뉴의 `Copy/Paste as → Copy link to selection`을 사용해도 됩니다. Trace Studio에는 `node-id`가 포함된 링크를 입력해야 합니다.
...
- `getNodeByIdAsync()`와 `JSON_REST_V1`을 이용한 현재 snapshot
