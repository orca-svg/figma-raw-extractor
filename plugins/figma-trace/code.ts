...
async function screenshot(node: SceneNode, maxDimension: number, maxBytes: number): Promise<ArtifactPayload | undefined> {
  ...
  try {
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      if (data.byteLength <= maxBytes) return { slot: "screenshot", kind: "screenshot", mimeType: "image/png", name: `${safeName(node.name)}.png`, data };
      scale *= .65;
    }
    return undefined;
  } catch {
...
async function sourceAssets(nodes: SceneNode[], options: BridgeJob["options"]): Promise<ArtifactPayload[]> {
...
async function extract(job: BridgeJob): Promise<{ result: PluginResult; payloads: ArtifactPayload[] }> {
  ...
  if (figma.fileKey !== job.target.fileKey) throw new Error("열린 Figma 파일과 Trace Studio 링크의 file key가 다릅니다.");
  const expectedEditor = job.target.fileType === "design" ? "figma" : "figjam";
  if (pluginMeta().editorType !== expectedEditor) throw new Error(`${job.target.fileType === "design" ? "Figma Design" : "FigJam"} 파일에서 플러그인을 열어 주세요.`);
  ...
  const partialByCount = totalKnown > job.options.maxNodes;
  const rawSnapshot = await node.exportAsync({ format: "JSON_REST_V1" });
  let pruned = pruneSnapshot(rawSnapshot, job.options.maxNodes);
  let serialized = JSON.stringify(pruned.value);
  while (utf8ByteLength(serialized) > job.options.maxJsonBytes && pruned.kept > 100) {
    pruned = pruneSnapshot(rawSnapshot, Math.max(100, Math.floor(pruned.kept / 2)));
    serialized = JSON.stringify(pruned.value);
  }
  if (utf8ByteLength(serialized) > job.options.maxJsonBytes) throw new Error("선택 노드 JSON이 20MB를 넘습니다. 더 좁은 레이어 링크를 사용해 주세요.");
  ...
  const partial = partialByCount || pruned.kept < totalKnown;
  const result: PluginResult = {
    snapshot: pruned.value,
    nodeCount: pruned.kept,
    partial,
    omittedNodes: partial ? Math.max(1, totalKnown - pruned.kept) : undefined,
